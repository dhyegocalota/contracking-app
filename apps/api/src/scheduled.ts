...
export async function handleScheduled(env: Environment): Promise<void> {
  ...
  await Promise.all(activeContractions.map(async (contraction) => {
    ...
    const existingLog = await env.DATABASE.prepare(SELECT_PUSH_NOTIFICATION_LOG)
      .bind(contraction.id, level)
      .first();
    ...
    await env.DATABASE.prepare(INSERT_PUSH_NOTIFICATION_LOG)
      .bind(crypto.randomUUID(), contraction.id, level)
      .run();
  }));
}
